...
const ensureDefaultWorkflowTables = async () => {
  ...
  if (!defaultWorkflowPromise) {
    defaultWorkflowPromise = (async () => {
      ...
      `);
      await pool.query(`CREATE INDEX IF NOT EXISTS idx_default_workflow_steps_sort ON default_workflow_steps(sort_order)`);
      await pool.query(`CREATE INDEX IF NOT EXISTS idx_default_workflow_tasks_step ON default_workflow_tasks(step_id)`);
      await pool.query(`CREATE INDEX IF NOT EXISTS idx_default_workflow_tasks_sort ON default_workflow_tasks(step_id, sort_order)`);

      const stepsCount = await pool.query('SELECT COUNT(*) FROM default_workflow_steps');
      if (parseInt(stepsCount.rows[0].count) === 0) {
        ...
        const bodyId = (await pool.query("SELECT step_id FROM default_workflow_steps WHERE step_name = 'Body'")).rows[0].step_id;
        const neckId = (await pool.query("SELECT step_id FROM default_workflow_steps WHERE step_name = 'Neck'")).rows[0].step_id;
        const paintId = (await pool.query("SELECT step_id FROM default_workflow_steps WHERE step_name = 'Paint Processing'")).rows[0].step_id;
        const releaseId = (await pool.query("SELECT step_id FROM default_workflow_steps WHERE step_name = 'Release'")).rows[0].step_id;

        await pool.query(
          `INSERT INTO default_workflow_tasks (step_id, task_name, sort_order)
           VALUES ($1, 'Shape Carving', 1), ($1, 'Pickup Cavity', 2), ($1, 'Electronics Cavity', 3), ($1, 'Neck Pocket', 4)`,
          [bodyId]
        );

        await pool.query(
          `INSERT INTO default_workflow_tasks (step_id, task_name, sort_order)
           VALUES ($1, 'Shape Carving', 1), ($1, 'Install Frets', 2), ($1, 'Drill Tuning Peg Holes', 3)`,
          [neckId]
        );

        await pool.query(
          `INSERT INTO default_workflow_tasks (step_id, task_name, sort_order)
           VALUES ($1, 'Sanding', 1), ($1, 'Primer', 2), ($1, 'Base Color', 3), ($1, 'Top Coat', 4), ($1, 'Buffing', 5), ($1, 'Polishing', 6)`,
          [paintId]
        );
      ...
      defaultWorkflowPromise = null;
      throw error;
    });
  }
  await defaultWorkflowPromise;
};
